  ...
  for(let i = 0; i < params.cols; i++){
    for(let j = 0; j < params.rows; j++){
      ...
      if(params.arr[i][j][0] != 0) {
        const [hue, saturation, lightness] =  hsvToHsl(params.arr[i][j][0],params.arr[i][j][1],params.arr[i][j][2])
        context.fillStyle = `hsl(${hue}, ${saturation}%, ${lightness}%)`
        context.fillRect(i * params.cellSize, j * params.cellSize, params.cellSize, params.cellSize)
      }
    }
  }
...
function updateSand(){
  ...
  for(let i = 0; i < params.cols; i++){
    for(let j = 0; j < params.rows - 1; j++){
      ...
      }else if(i < params.cols - 1 && i > 0){
        if(previousArr[i - direction][j + 1][0] == 0){
          params.arr[i][j] = new Array(3).fill(0);
          params.arr[i - direction][j + 1] = previousArr[i][j];
        } else if(previousArr[i + direction][j + 1][0] == 0){
          params.arr[i][j] = new Array(3).fill(0);
          params.arr[i + direction][j + 1] = previousArr[i][j];
        }
      }
...
const startSketch = async () => {
  ...
  canvas.addEventListener('mousedown', function(event) {
    const rect = canvas.getBoundingClientRect();
      ...
      if(col >= 0 && col <= params.cols && row >= 0 && row <= params.rows && params.arr[col][row][0] == 0) params.arr[col][row] = [params.h,params.s,params.v];
